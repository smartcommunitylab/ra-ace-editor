...
import { SimpleList, SimpleForm, SimpleShowLayout, TextField, TextInput } from 'react-admin';
...
const dataProvider = fakeRestProvider(
    {
        posts: [
            { id: 1, title: 'Lorem Ipsum', body: "Some <strong>html</strong> text" },
            { id: 2, title: 'Sic dolor amet', body: "Some <strong>html</strong> text" },
        ],
    },
    true
);
...
export const FullApplication = () => (
    <Admin history={createMemoryHistory()} dataProvider={dataProvider}>
        <Resource name="posts"
            list={
                <List>
                    <SimpleList primaryText={(record: any): string => record.title} />
                </List>
            }
            edit={
                <Edit>
